...
public enum Transmogrifier {
    ...
    private static func updateVersionMin(_ data: Data, _ offset: UInt32, minos: UInt32, sdk: UInt32, platform: UInt32) -> Data {
    ...
    private static func computeLoadCommandsEditor(_ loadCommandsData: [Data], isDynamic: Bool) -> ((Data, UInt32, UInt32, UInt32) -> Data) {
    ...
    static func updatePostiOS12ObjectFile(lc: Data, minos: UInt32, sdk: UInt32, platform: UInt32) -> Data {
        let cmd = Int32(bitPattern: lc.loadCommand)
        switch cmd {
        case LC_BUILD_VERSION:
            return updateVersionMin(lc, 0, minos: minos, sdk: sdk, platform: platform)
        default:
            return lc
        }
    }
    
    static func updatePreiOS12ObjectFile(lc: Data, minos: UInt32, sdk: UInt32, platform: UInt32) -> Data {
        // `offset` is kind of a magic number here, since we know that's the only meaningful change to binary size
        // having a dynamic `offset` requires two passes over the load commands and is left as an exercise to the reader
        ...
        case LC_VERSION_MIN_IPHONEOS:
            return updateVersionMin(lc, offset, minos: minos, sdk: sdk, platform: platform)
    ...
    static func updateDylibFile(lc: Data, minos: UInt32, sdk: UInt32, platform: UInt32) -> Data {
        // `offset` is kind of a magic number here, since we know that's the only meaningful change to binary size
        // having a dynamic `offset` requires two passes over the load commands and is left as an exercise to the reader
        let offset = UInt32(abs(MemoryLayout<build_version_command>.stride - MemoryLayout<version_min_command>.stride))
        let cmd = Int32(bitPattern: lc.loadCommand)
    ...
    public static func processBinary(atPath path: String, minos: UInt32 = 13, sdk: UInt32 = 13, platform: Platform, isDynamic: Bool = false) {
        ...
        let editedCommandsData = loadCommandsData
            .map { return editor($0, minos, sdk, UInt32(platform.rawValue)) }
            .merge()
